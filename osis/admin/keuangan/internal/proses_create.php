...
move_uploaded_file($tmp_name, "nota/". $nama_baru);
            ...
            $sql    = "SELECT * from keuangan_internal where id_internal IN(SELECT MAX(id_internal) FROM keuangan_internal)";
...
echo "<br>";
	...
	if (empty($pengeluaran)){
		...
		$sql = "INSERT INTO keuangan_internal (pemasukan, saldo, tanggal, keterangan, bukti) VALUES ('$pemasukan', '$saldo', '$tanggal', '$keterangan', '$nama_baru')";
		mysqli_query($koneksi,$sql);
		header('location:index.php');
	}else{

		echo $saldo = ($kas - $pengeluaran);
		$sql = "INSERT INTO keuangan_internal (pengeluaran, saldo, tanggal, keterangan, bukti) VALUES ('$pengeluaran', '$saldo', '$tanggal', '$keterangan', '$nama_baru')";
		mysqli_query($koneksi,$sql);
		header('location:index.php');
	}
