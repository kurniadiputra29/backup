			...
			<div v-for="(order, index) in orders" :key="index">
				<div class="form-group row"  >
					...
					<div class="col-sm-10" style="margin-top: 10px;">
						<textarea name="note[]" :value="note(order.product_id, index)" class="form-control" rows="3" placeholder="Enter ..." id="note" >
						</textarea>
					</div>
...
<script type="text/javascript">
	new Vue({
		...
		methods: {
			...
			note(product_id, index){
				var name =  this.productsname[product_id];
				this.orders[index].name = name;
				return name;
			},
